...
def processar_dados(df_horas_raw, df_mov_raw, horas_mes):
    ...
    # Corretiva por equipamento
    corr_eq = df[df['TIPO']=='Corretiva'].groupby('cod').agg(
        HORAS=('UTT','sum'), OS=('P_ORDEM','nunique')
    ).reset_index().merge(eq_info, on='cod').sort_values('HORAS', ascending=False).head(8)
    ...
    return {
        ...
        'corr_eq': corr_eq.to_dict('records'),
        'fds_h': round(fds_h, 1),
        'horas_mes': horas_mes,
    }
...
def gerar_html(d, nome, cargo):
    ...
    tech_js = json.dumps([{
        'nome': r['NOME'].title(),
        'tipo': 'Elétrica' if 'ELET' in str(r.get('NP','')) or i % 2 == 1 else 'Mecânica',
        'horas': r['HORAS'],
        'occ': r['OCC'],
        'saldo': r['SALDO'],
    } for i, r in enumerate(d['colab'])])
    ...
    if d['pm_pct'] >= 50:
        diag_pontos_pos += f'<div class="diag-item"><div class="diag-dot dot-green"></div><span>{d["pm_pct"]}% das horas em manutenção preventiva demonstra comprometimento com o plano mesmo sob pressão de corretivas.</span></div>'
    if d['occ_media'] <= 100:
        diag_pontos_pos += f'<div class="diag-item"><div class="diag-dot dot-green"></div><span>Ocupação média de {d["occ_media"]}% — equipe dentro da capacidade nominal com margem disponível.</span></div>'
    diag_pontos_pos += f'<div class="diag-item"><div class="diag-dot dot-green"></div><span>{d["total_os"]} OS gerenciadas por {d["total_tec"]} técnicos no mês — alta produtividade operacional.</span></div>'

    if d['cm_pct'] > 25:
        diag_pontos_neg += f'<div class="diag-item"><div class="diag-dot dot-red"></div><span>{d["cm_pct"]}% de corretiva é acima do ideal (≤25%). Equipamentos críticos operam com PM insuficiente.</span></div>'
    if d['occ_media'] > 100:
        diag_pontos_neg += f'<div class="diag-item"><div class="diag-dot dot-red"></div><span>Todos os técnicos ultrapassaram {d["horas_mes"]}h. Total de {d["excedente_total"]}h excedentes — sem margem para emergências.</span></div>'
    if d['pm_pct'] < 60:
        diag_pontos_neg += f'<div class="diag-item"><div class="diag-dot dot-yellow"></div><span>Preventiva abaixo da meta de 60% ({d["pm_pct"]}%). Elevar gradualmente nos próximos meses.</span></div>'
...
<div class="section">
  ...
  <div class="kpi-grid">
    ...
    <div class="kpi-card">
      ...
      <div class="kpi-value">{total_mat_fmt}</div>
...
<div class="section">
  ...
  <p style="font-size:11px;color:var(--muted);margin-top:8px;">★ Top 5 equipamentos concentraram {d['top10'][4]['ACUM'] if len(d['top10'])>=5 else '—'}% de todas as horas.</p>
</div>
...
new Chart(document.getElementById('chartTipo'), {{
  type:'doughnut',
  data:{{
    labels:['Preventiva','Corretiva','Geral/Apoio'],
    datasets:[{{data:[{d['pm_h']},{d['cm_h']},{d['ge_h']}],backgroundColor:['#1a3a5c','#c8510a','#b4b2a9'],borderWidth:2,borderColor:'#ffffff'}}]
  }},
...
new Chart(document.getElementById('chartSetor'), {{
  ...
  data:{{
    labels:{setor_labels},
    datasets:[{{label:'Horas',data:{setor_data},backgroundColor:'#1a3a5c',borderRadius:3,borderWidth:0}}]
  }},
    ...
    if st.button("⚙️ Gerar Relatório"):
        with st.spinner("Processando dados..."):
            try:
                ...
                with col1:
                    st.metric("Preventiva", f"{dados['pm_pct']}%", f"{dados['pm_h']}h")
                with col2:
                    st.metric("Corretiva",  f"{dados['cm_pct']}%", f"{dados['cm_h']}h", delta_color="inverse")
                with col3:
                    st.metric("Ocupação equipe", f"{dados['occ_media']}%", f"+{dados['excedente_total']}h" if dados['excedente_total']>0 else "OK", delta_color="inverse" if dados['excedente_total']>0 else "normal")
